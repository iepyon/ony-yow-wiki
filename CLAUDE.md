...
| やりたいこと | 手段 |
|---|---|
| 朝: 今日やることを話す | `/ony` → **Y から逆算**して O/N/O2 を聞き出し、カード作成 → `day/<日付>` ブランチ → **ドラフト PR** まで |
| 夕: 結果と教訓を話す | `/yow` → 今朝の Y 一覧から O（結果）と W（教訓）を聞き出し、**同じドラフト PR に積む** |
| 日を確定する | 本人が `gh pr ready` → マージ（**マージ = 日の確定**） |
