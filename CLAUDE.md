...
- **画像をリンクで包まない・表を `<details>` で畳まない** — API 経由の投稿では書き換えられる。
  リンク付き画像 `[![](img)](url)` はバッククォートで括られて文字列になり、`<details>` は
  タグごと落ちる。スライドへの導線は表の後に素のリンクで置く（2026-08-15 に PR #6 で観測）
