...
- **ONY カードを書いたら即・単独コミット**。YOW は別コミット
  — コミットの分離が「予測を結果より先に書いた」ことの唯一の証明
...
| やりたいこと | 手段 |
|---|---|
| 朝: 今日やることの根拠を記録 | `/ony` → ONY カード作成 + gen_deck + 即コミット |
| 夕: 結果どうだったかを記録 | `/yow` → 未検証一覧から選び YOW カード作成 + gen_deck + 別コミット |
| 週を締める | `/close-week` → 前週デッキの stable 化差分を PR に |
| 週締め PR にレビューコメント | `/onyw-review <PR番号>`（バイアス / 因果の弱さ / 別の選択肢） |
| 一覧・バックリンクを見る | `npx --prefix $SW tsx $SW/src/cli.ts --wiki wiki out/index.html` |

**週締め = PR。マージ = 週の確定。** レビューの往復は PR コメントに残す（A3 のキャッチボール）。
