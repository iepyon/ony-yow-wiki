...
**PR の状態で力点を変える**（日次 PR は朝と夕の2回レビューされる）:
- **朝（ONY のみ・YOW カード未着）= 計画レビュー**。まだ直せる段階なので、
  ①は確証・利用可能性（O の選び方。後知恵検査は対象外）、②は「O2 は Y から本当に
  出る予測か」（事前の因果検査）、③は N←Y と「ひとつ外すなら」（今日の Y の削り）に力点。
- **夕（YOW 済み）= 検証レビュー**。現行の表どおり（後知恵検査・O2/O の食い違いの W 反映を含む）。
  朝コメントへの応答が W に現れているかも見る。
...
3. **集約して1コメントに** — サブエージェントの結果から**全体で最も刺さる3件だけ**残す
   （原則each観点から1件。該当が無い観点は他観点から補充）。
   **各指摘は2行以内**（指摘1文 → 問い1文）。専門語（〜ヒューリスティック等）は平易な言葉に開く。
   ...
   *履歴: <ony→yow の順序と改変有無を1行>。マージ判断は本人。*
   ```
